...
import { buildCallSummary, sendTelegramMessage } from "./notify.mjs";
...
<Response>
  ...
  // Track which calls we've already sent a notification for (prevent duplicates
  // when Twilio fires /call-status multiple times or after WebSocket close)
  const notified = new Set();

  async function maybeNotifyTelegram({ callSid, to, status, reason, notifyContext }) {
    if (notified.has(callSid)) return;
    notified.add(callSid);
    if (!telegramBotToken || !telegramChatId) {
      req.log?.warn({ callSid }, "Notify requested but Telegram bot not configured");
      return;
    }
    try {
      const summary = await buildCallSummary({
        callSid, to, status, reason, notifyContext, dataDir,
        llmApiUrl, llmApiToken, llmModel,
        personaPrompt: defaultSystemPrompt,
      });
      await sendTelegramMessage({ botToken: telegramBotToken, chatId: telegramChatId, text: summary });
      console.error(`[notify] sent Telegram summary for ${callSid} (${status})`);
    } catch (e) {
      console.error(`[notify] failed for ${callSid}: ${e.message}`);
    }
  }
  ...
  app.post("/call-status", async (req, reply) => {
    ...
    const notifyContext = req.query?.notify_context;
    ...
    if (TERMINAL_STATUSES.includes(CallStatus)) {
      ...
      if (notify) {
        // Delay slightly so the WebSocket close handler finishes writing the transcript
        setTimeout(() => {
          maybeNotifyTelegram({
            callSid: CallSid,
            to: To || call?.to,
