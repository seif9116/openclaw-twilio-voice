...
export function createMcpServer(config) {
  ...
  server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
      {
        name: "make_call",
        description: "Make an outbound phone call. You (Jessica) will be the one on the call — provide a reason so you know why you called once the person picks up. Set notify_on_end=true if you want a summary sent to Seif via Telegram after the call.",
        inputSchema: {
          type: "object",
          properties: {
            ...
            notify_on_end: { type: "boolean", description: "If true, after the call ends a Telegram message is sent to Seif summarizing what happened. Use this when Seif asks you to call someone on his behalf and wants to hear back." },
            notify_context: { type: "string", description: "What Seif specifically asked you to find out. Used to frame the follow-up summary. Example: 'whether they can do Friday at 3pm'" },
          },
          required: ["to", "reason"],
        },
      },
  ...
  server.setRequestHandler(CallToolRequestSchema, async (req) => {
    ...
    if (name === "make_call") {
      const { to, reason, greeting, notify_on_end, notify_context } = req.params.arguments;
      try {
        // Encode context in both URLs so the webhook server (separate process) can read it
        // The StatusCallback URL receives no-answer/failed cases where outbound-twiml never fires
        ...
        if (notify_on_end) twimlUrl.searchParams.set("notify", "1");
        if (notify_context) twimlUrl.searchParams.set("notify_context", notify_context);

        const statusCallbackUrl = new URL(`${publicUrl}/call-status`);
        if (notify_on_end) statusCallbackUrl.searchParams.set("notify", "1");
        if (notify_context) statusCallbackUrl.searchParams.set("notify_context", notify_context);
        if (reason) statusCallbackUrl.searchParams.set("reason", reason);
